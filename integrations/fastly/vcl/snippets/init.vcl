...
sub cheq_rti_recv {
    ...
    if (req.method == "POST" && req.url.qs ~ "(?:^|&)captcha=([0-9a-f]+)(?:$|&)") {
        ...
        if (var.submitted_nonce == var.expected_nonce) {
            ...
            set req.backend                    = F_cheq_captcha_backend;
            return(pass);
        }
...
sub cheq_rti_backend_fetch {
    ...
    # NOTE: The origin_host Host-header override is intentionally absent here.
    # In the snippet deployment model pass.vcl calls this subroutine and then
    # applies the override itself (covering both req.restarts >= 1 and
    # req.http.X-Cheq-Session-Bypass).  See snippets/pass.vcl.
    # In the standalone deployment model (cheq_rti.vcl) the override lives
    # inside this subroutine instead.
